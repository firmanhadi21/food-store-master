...
- **食料品店マスター**（スーパー・食料品店・コンビニ・ドラッグストア等）を、**Overture Places と OpenStreetMap の2ソースから**統合構築
  （実測: `src` 列は overture 97,600 / osm 5,384 の2値のみ。**食品営業許可オープンデータはマスターに投入していない**＝検証用。下記「データソースとライセンス」参照）
...
| 食品営業許可オープンデータ | **検証のみ**（マスターに入らない） | 各自治体（保健所）／ **厚生労働省**「食品衛生申請等システム（FAS）」 | 各データのオープンライセンス |
